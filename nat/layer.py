...
from .modules import FeedForward, RelativeSelfAttention, RelativePositionEmbeddings, JYCMultiheadAttention
...
class BlockedEncoderLayer(TransformerEncoderLayer):
    ...
    def build_self_attention(self, embed_dim, args):
        if getattr(args, "enc_self_attn_cls", "abs") == "abs":
            return MultiheadAttention(
                ...
                bias=not getattr(args, "enc_no_bias_for_self_attn", False),
            )
        else:
    ...
    def forward(self, x, encoder_padding_mask, attn_mask: Optional[Tensor] = None, need_head_weights=False):
...
class BlockedDecoderLayer(TransformerDecoderLayer):
    ...
    def build_self_attention(self, embed_dim, args, add_bias_kv=False, add_zero_attn=False):
        if getattr(args, "self_attn_cls", "abs") == "abs":
            return MultiheadAttention(
                ...
                q_noise=self.quant_noise,
                qn_block_size=self.quant_noise_block_size,
                bias=not getattr(args, "dec_no_bias_for_self_attn", False),
            )
        else:
            return RelativeSelfAttention(
                embed_dim,
    ...
    def build_encoder_attention(self, embed_dim, args):
        return JYCMultiheadAttention(
            ...
            q_noise=self.quant_noise,
            qn_block_size=self.quant_noise_block_size,
            bias=not getattr(args, "dec_no_bias_for_cross_attn", False),
        )
    ...
    def forward(
        ...
        if self.cross_self_attention and not (
            ...
            assert False, "JYC: not used."
            if self_attn_mask is not None:
                assert encoder_attn_key is not None
                self_attn_mask = torch.cat(
                    (x.new_zeros(x.size(0), encoder_attn_key.size(0)), self_attn_mask), dim=1
                )
            if self_attn_padding_mask is not None:
                if encoder_padding_mask is None:
                    assert encoder_attn_key is not None
                    encoder_padding_mask = self_attn_padding_mask.new_zeros(
                        encoder_attn_key.size(1), encoder_attn_key.size(0)
                    )
                self_attn_padding_mask = torch.cat(
                    (encoder_padding_mask, self_attn_padding_mask), dim=1
                )
            assert encoder_attn_key is not None and encoder_attn_value is not None
            k = torch.cat((encoder_attn_key, x), dim=0)
            v = torch.cat((encoder_attn_value, x), dim=0)
        else:
            k = x
            v = x
            ...
            if self.relative_keys is None:
                ...
                if incremental_state is not None:
                    ...
                    if "prev_key" in saved_state:
                        ...
                        index = utils.new_arange(x, prev_key.size(1) + k.size(0))
                    else:
                        index = utils.new_arange(x, x.size(0))
                else:
                    index = utils.new_arange(x, x.size(0))
        ...
        if not self.normalize_before:
            x = self.final_layer_norm(x)
        return_attn_type = 'logits' if need_attn_logits else 'weights'
        self_attn_to_return = attn[1][return_attn_type]
        cross_attn_to_return = cross_attn[return_attn_type] if cross_attn is not None else None
        if self.onnx_trace and incremental_state is not None:
            ...
            return x, {'self_attn': self_attn_to_return, 'cross_attn': cross_attn_to_return}, self_attn_state

        return x, {'self_attn': self_attn_to_return, 'cross_attn': cross_attn_to_return}, None
...
class ContentTrainer(nn.Module):
    ...
    def _compute_state_loss(self, x, tgt_tokens, mask=None, window_size=-1):
        ...
        print('left', left)
        print('right', utils.new_arange(score, window_size + 1))
